...
def get_base_url(provided_url: str | None) -> str | None:
    """Get Gemini base URL from argument first, then environment."""
    if provided_url:
        return provided_url
...
def main():
    ...
    parser.add_argument(
        "--input-image", "-i",
        nargs="+",
        help="One or more input image paths for editing/reference (e.g., -i img1.png img2.png)"
    )
    parser.add_argument(
        "--resolution", "-r",
        choices=["1K", "2K", "4K"],
    ...
    parser.add_argument(
        "--base-url", "-b",
        help="Gemini API base URL (overrides GEMINI_BASE_URL env var)"
    )
